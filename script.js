...
@import url('https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&family=Mulish:wght@400;500;600;700;800&family=Playfair+Display:wght@700;800&display=swap');
...
:root{
  ...
  --display-header:"Playfair Display",Georgia,serif;
  --r:12px; --r-sm:8px;
...
/* ---------- page title / masthead ---------- */
.masthead{
  position:relative; overflow:hidden; border-radius:20px; margin:26px 0 30px;
  padding:38px 34px 42px; box-shadow:var(--sh-lg);
...
.masthead-logo{flex:none; width:84px; height:84px; filter:drop-shadow(0 4px 10px rgba(0,0,0,.25)); transform:scaleX(-1)}
.masthead-logo-tail, .masthead-logo-wing{fill:rgba(255,255,255,.55)}
.masthead-logo-body, .masthead-logo-head, .masthead-logo-beak{fill:#fff}
.masthead-logo-eye{fill:var(--forest)}
h1{
...
.mutation-columns{display:grid; grid-template-columns:150px minmax(0,1fr); gap:15px; width:100%; align-items:center}
.symbol-col{
...
button#share-btn:hover{transform:translateY(-2px); box-shadow:var(--sh-md)}
button#share-btn:active{transform:translateY(0)}
...
.live-preview-actions{
  display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:14px; padding-top:14px;
...
.mobile-preview-arrow{flex:none; color:var(--grass); font-size:.8em}
...
.mobile-preview-expanded .live-preview-actions button{padding:10px 12px; font-size:.82em}
...
@media (max-width:820px){
  ...
  .mutation-columns{grid-template-columns:96px minmax(0,1fr); gap:8px}
...
@media (max-width:600px){
  ...
  .masthead{padding:30px 20px 34px; margin:16px 0 22px}
  ...
  .z-assign label, .t-assign label{padding:5px 5px; margin:0 1px; font-size:.6em; min-width:0}
  .linkage-label{font-size:.66em; margin-right:4px; white-space:nowrap}
  ...
  /* The bottom "Hide/Show Genetic Symbols" button sits directly above
     ...
     compact size. */
  button#calc-btn, button#reset-btn, button#toggle-symbols-btn-2{width:100%}
  #controls-container button{margin:0 0 8px !important}
  button#toggle-symbols-btn{
    width:auto; display:inline-block; padding:9px 16px; font-size:.8em;
  }

  /* results tables -> stacked cards (kept from the original 600px behaviour,
     extended to the offspring tables so formulas with sup/sub stay readable) */
  ...
  #results-content table, #results-content thead, #results-content tbody,
  #results-content th, #results-content td, #results-content tr{display:block; width:100%}
  #results-content thead{display:none}
  #results-content table{border:1px solid var(--line); border-radius:var(--r-sm); overflow:hidden; margin-bottom:18px}
  #results-content tr{padding:10px 12px; border-bottom:1px solid var(--line)}
  #results-content tr:last-child{border-bottom:none}
  #results-content td{padding:4px 0; border:none}
  #results-content td.col-genetic-formula{font-size:1.05em; padding-bottom:6px}

  /* The stacked-card layout above hides <thead> since a header row
     doesn't mean much once columns collapse to vertical blocks -- but
     that left every value with no label at all. Repeat the column
     heading as a small caption above each value instead (the standard
     CSS-only pattern for responsive tables), keyed by column position
     since these cells don't carry per-column classes to hook into. */
  .parents-table td::before, #results-content td::before{
    display:block; font-size:.66em; font-weight:800; text-transform:uppercase;
    letter-spacing:.05em; color:var(--ink-soft); margin-bottom:3px;
  }
  .parents-table td:nth-child(1)::before{content:"Genotype / Mutation Name"}
  .parents-table td:nth-child(2)::before{content:"Genetic Formulas"}
  #results-content td:nth-child(1)::before{content:"Genotype / Mutation Name"}
  #results-content td:nth-child(2)::before{content:"Probability"}
  #results-content td:nth-child(3)::before{content:"Genetic Formulas"}
}
...
/* ============================================================================
   WIDE TIER (>=1600px) — reveal the sticky live-preview panel to the right
   of the Dam column. Page max-width grows to 1700px: 440px panel + 20px gap
   on top of the original 1240px two-column width -- chosen specifically so
   Sire/Dam end up exactly their original (pre-feature) width rather than
   needing to shrink to make room for the wider panel.
   Scoped to .container/#results-container/#shared-banner only -- NOT the
   masthead -- so the hero/tagline keep their current, more readable line
   length rather than stretching to match. Below this width (a large amount
   of real "desktop" traffic is a 1280-1440px laptop window, not a 1920px
   monitor) the layout quietly falls back to today's behavior: no panel,
   #controls-container is the only way to act.
   ========================================================================== */
@media (min-width:1600px){
  body{max-width:1700px}
  .masthead, .header-controls{max-width:1240px; margin-left:auto; margin-right:auto}
  ...
  /* The original Reset/Generate/Toggle row stays -- both #controls-container
     ...
     (total width minus the 440px panel and the one 20px gap before it)
     keeps its centered button row aligned under Sire+Dam only, instead of
     drifting off-center under the full, wider page. */
...
@media (max-width:600px){
  ...
  /* Keeps the last mutation item (and the validation reminder / results
     ...
     growing the page, so it doesn't need to be accounted for here. */
